...
def extract_near_surface_volume_fn(input_tensor: torch.Tensor, alpha: float):
    ...
    valid_mask = val > -9000  # Assume -9000 marks invalid values

    # Improved neighbor retrieval function (preserves dimension consistency)
    def get_neighbor(t, shift, axis):
        """Shift along the specified axis while keeping dimensions consistent"""
        if shift == 0:
            return t.clone()

        # Determine padding axis (input is [D, D, D] corresponding to z, y, x axes)
        ...
        # Perform padding (add batch and channel dims for F.pad compatibility)
        padded = F.pad(t.unsqueeze(0).unsqueeze(0), pad_dims[::-1], mode='replicate')  # Reverse order for F.pad

        # Build dynamic slice indices
        slice_dims = [slice(None)] * 3  # Initialize with full slices
    ...
    # Get neighbors in each direction (ensuring dimension consistency)
    ...
    # Handle boundary invalid values (use torch.where to preserve dimensions)
    ...
    # Compute sign consistency (cast to float32 for precision)
